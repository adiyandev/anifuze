...
export async function createSupportTicket({adminUserId,subject,message,priority='normal'}){
 ...
 if(process.env.DB_CLIENT==='postgres')await query('INSERT INTO af_support_tickets(id,admin_user_id,subject,message,priority) VALUES($1,$2,$3,$4,$5)',[ticketId,adminUserId,s,m,p]);else await query('INSERT INTO af_support_tickets(id,admin_user_id,subject,message,priority) VALUES(?,?,?,?,?)',[ticketId,adminUserId,s,m,p]);
 const ticket=(await query('SELECT * FROM af_support_tickets WHERE id=$1',[ticketId])).rows[0]; const webhook=String(process.env.ANIFUZE_SUPPORT_WEBHOOK_URL||'').trim();
...
export async function updateSupportTicket(id,status){const allowed=['open','pending','resolved','closed'];if(!allowed.includes(status))throw new Error('Invalid ticket status.');await query('UPDATE af_support_tickets SET status=$1,updated_at=CURRENT_TIMESTAMP WHERE id=$2',[status,id]);return (await query('SELECT * FROM af_support_tickets WHERE id=$1',[id])).rows[0]||null;}

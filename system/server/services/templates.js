...
import {fetchTemplatePackageMetadata,downloadVerifiedPackage} from './templatePackage.js';
...
export async function installTemplate(id){
 ...
 await query('INSERT INTO af_templates(id,name,version,status,config,installed_at,updated_at,package_url,package_sha256,package_signature,package_size,compatibility,description,category) VALUES($1,$2,$3,$4,$5,$6,$6,$7,$8,$9,$10,$11,$12,$13) ON CONFLICT(id) DO UPDATE SET name=$2,version=$3,status=$4,config=$5,updated_at=$6,package_url=$7,package_sha256=$8,package_signature=$9,package_size=$10,compatibility=$11,description=$12,category=$13',[remote.id,remote.name,remote.version,'installed',JSON.stringify(remote.config),now,remote.packageUrl,remote.packageSha256,remote.packageSignature,packageBuffer.length,JSON.stringify(remote.compatibility),remote.description,remote.category]);
 return getInstalledTemplate(remote.id);
}
export async function getInstalledTemplate(id){const r=await query('SELECT id,name,version,status,config,installed_at,updated_at,package_url,package_sha256,package_signature,package_size,compatibility,description,category FROM af_templates WHERE id=$1',[String(id)]);const x=r.rows[0];return x?{...x,config:parse(x.config),compatibility:parse(x.compatibility)}:null;}
export async function listInstalledTemplates(){const r=await query('SELECT id,name,version,status,config,installed_at,updated_at FROM af_templates ORDER BY name ASC');return r.rows.map(x=>({...x,config:parse(x.config),compatibility:parse(x.compatibility)}));}

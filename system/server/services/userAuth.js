...
export async function createUserSession(user,{req,remember=true}){
 const id=crypto.randomBytes(32).toString('base64url'),now=new Date(),expires=new Date(now.getTime()+(remember?LONG_SESSION_MS:SHORT_SESSION_MS));
 await query('INSERT INTO af_user_sessions(id,user_id,expires_at,created_at,last_seen_at,ip_address,user_agent,remember_me) VALUES($1,$2,$3,$4,$4,$5,$6,$7)',[id,user.id,expires.toISOString(),now.toISOString(),req.ip,String(req.headers['user-agent']||'').slice(0,1000),Boolean(remember)]);
 await query('UPDATE af_users SET last_active_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=$1',[user.id]);
 return {id,expires,user:publicUser(user),remember:Boolean(remember)};
}

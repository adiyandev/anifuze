...
export function PublicLayout(){
 ...
 useEffect(()=>{let active=true;fetch('/api/template').then(r=>r.ok?r.json():Promise.reject()).then(d=>{if(!active)return;const next=d.template||null;setTemplate(next);const cfg=next?.config&&typeof next.config==='object'?next.config:{};const root=document.documentElement;const vars:any={primary:cfg.primary||cfg.primaryColor,accent:cfg.accent||cfg.accentColor,background:cfg.background||cfg.backgroundColor,surface:cfg.surface||cfg.surfaceColor,text:cfg.text||cfg.textColor,muted:cfg.muted||cfg.mutedColor,radius:cfg.radius||cfg.cardRadius,font:cfg.font||cfg.fontFamily};Object.entries(vars).forEach(([key,value])=>{if(value!=null&&String(value).trim())root.style.setProperty('--vault-'+key,String(value))})}).catch(()=>{if(active)setTemplate(null)});return()=>{active=false}},[]);
 ...
 return <div className="vault-shell" data-template={template?.id||undefined} style={{'--vault-primary':settings.primary,'--vault-accent':settings.accent} as CSSProperties}>

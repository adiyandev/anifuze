...
interface Release{version:string;tag:string;url:string;name:string;publishedAt:string|null;notes:string;assets:{name:string;size:number;url:string;sha256?:string}[]}
...
export function UpdatesPage(){
 ...
 const stage=async()=>{const asset=result?.release.assets.find(a=>a.sha256);if(!asset||!result)return;setChecking(true);setError('');try{const r=await fetch('/api/admin/updates/release/stage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({version:result.release.version,packageUrl:asset.url,sha256:asset.sha256})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Failed to stage release');setState(j.state);setSaved('Release downloaded, verified, and staged. A pre-upgrade backup was created.');}catch(e){setError(e instanceof Error?e.message:'Failed to stage release');}finally{setChecking(false);}};
 const clearStage=async()=>{setChecking(true);try{const r=await fetch('/api/admin/updates/release/stage',{method:'DELETE'});const j=await r.json();if(!r.ok)throw new Error(j.error||'Failed to clear staged release');setState(j.state);}catch(e){setError(e instanceof Error?e.message:'Failed to clear staged release');}finally{setChecking(false);}};

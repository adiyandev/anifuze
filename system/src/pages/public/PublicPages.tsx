...
export function HomePage(){
 ...
 const liveTrending=fresh.trending; const liveLatest=fresh.latest; const featured=liveTrending[0]||fresh.popular[0]; const slides=liveTrending.slice(0,5); const blocks=homepageBlocks.filter((b:any)=>!b.hidden);
 if(!featured)return <section className={`vault-home template-${template?.id||'default'}`}><div className="vault-container"><div className="vault-builder-text"><span className="vault-kicker">{settings.siteName.toUpperCase()}</span><h1>{fresh.trending.length===0?'Loading anime…':'No anime available'}</h1><p>{fresh.trending.length===0?'AniList is being queried for the latest catalog. Please try again in a moment.':'AniList returned no trending anime right now.'}</p><button className="vault-primary" onClick={()=>window.location.reload()}>Retry</button></div></div></section>;
...
function BuilderSection({block,liveAnime,latestAnime}:{block:any;liveAnime:any[];latestAnime:any[]}){const{settings}=useApp();const t=String(block.type||'').toLowerCase();if(t==='hero')return <div className="vault-builder-section vault-builder-hero"><span className="vault-kicker">CUSTOM HERO</span><h2>{block.title}</h2><p>{block.content}</p><Link className="vault-primary" to="/browse">Browse anime</Link></div>;if(['anime grid','anime_grid','anime carousel','anime_card','anime card','collection','stats'].includes(t))return <Shelf title={block.title} subtitle={block.content||block.description||''} items={animeOnly(block.items||liveAnime).slice(0,6)} episode={t==='anime card'||t==='anime_card'}/>;if(t==='episode list'||t==='schedule')return <Shelf title={block.title} subtitle={block.content} items={animeOnly(latestAnime).slice(0,6)} episode/>;if(t==='cta'||t==='banner')return <div className="vault-cta"><div><span>{settings.siteName.toUpperCase()}</span><h2>{block.title}</h2><p>{block.content}</p></div><Link className="vault-secondary" to="/browse">Explore library ↗</Link></div>;if(t==='navbar'||t==='footer')return null;return <section className="vault-builder-text"><span className="vault-kicker">{block.type}</span><h2>{block.title}</h2><p>{block.content}</p></section>} 
...
export function WatchPage(){
 ...
 const[progress,setProgress]=useState(0); const[stream,setStream]=useState<any>(null); const[loadingStream,setLoadingStream]=useState(false); const[streamError,setStreamError]=useState('');
 const loadStream=async(nextEp:number)=>{if(!id)return;setLoadingStream(true);setStreamError('');try{const r=await fetch('/api/stream/'+encodeURIComponent(id)+'?episode='+nextEp);const d=await r.json();if(!r.ok)throw new Error(d.error||'No playable stream found.');setStream(d);try{const key='anifuze_analytics_session';let session=sessionStorage.getItem(key);if(!session){session=crypto.randomUUID();sessionStorage.setItem(key,session)}fetch('/api/analytics/events',{method:'POST',headers:{'Content-Type':'application/json'},keepalive:true,body:JSON.stringify({event_type:'stream_start',session_id:session,anime_id:id,episode_id:String(nextEp),path:window.location.pathname})}).catch(()=>{})}catch{} }catch(e:any){setStream(null);setStreamError(e.message||'No playable stream found.')}finally{setLoadingStream(false)}};
 useEffect(()=>{let alive=true;if(id)fetchAnimeById(id).then(async x=>{if(alive){setAnimeData(x);const saved=await watchProgressService.get(id,1);if(saved)setProgress(saved.progressSeconds);await loadStream(1)}}).catch(()=>{});return()=>{alive=false}},[id]);
 ...
 const changeEpisode=async(next:number)=>{if(!animeData)return;await watchProgressService.save({animeId:id!,episode:ep,progressSeconds:progress,durationSeconds:1440});setEp(next);const saved=await watchProgressService.get(id!,next);setProgress(saved?.progressSeconds||0);await loadStream(next)};
 if(!animeData)return <section className="vault-page"><h1>Loading anime…</h1></section>;
 const percent=Math.min(100,(progress/1440)*100); const source=stream?.sources?.[0]; const playable=source&&/^(hls|mp4|video|auto)$/i.test(source.type||'');
 return <section className="vault-watch"><div className="vault-player">{loadingStream?<div className="vault-builder-text"><h2>Finding a stream…</h2><p>Checking the providers configured by the site administrator.</p></div>:source&&playable?<video key={source.url} className="vault-video" controls autoPlay playsInline src={source.url} onTimeUpdate={e=>setProgress(Math.floor((e.currentTarget.currentTime||0)))} onLoadedMetadata={e=>{if(Number.isFinite(e.currentTarget.duration)&&e.currentTarget.duration>0)setProgress(Math.min(progress,Math.floor(e.currentTarget.duration)))}}/>:source?<div className="vault-builder-text"><h2>External playback source</h2><p>{source.providerName} supplied a playback URL that cannot be rendered as a native video.</p><a className="vault-primary" href={source.url} target="_blank" rel="noreferrer">Open source</a></div>:<div className="vault-builder-text"><h2>Stream unavailable</h2><p>{streamError||'No enabled provider returned a playable source.'}</p><button className="vault-primary" onClick={()=>loadStream(ep)}>Retry</button></div>}<div className="watch-progress"><i style={{width:percent+'%'}}/></div></div><div className="vault-watch-head"><div><span className="vault-kicker">NOW PLAYING</span><h1>{animeData.title}</h1><p>Episode {ep} · {animeData.genre} · {Math.floor(progress/60)}:{String(progress%60).padStart(2,'0')} watched</p></div><div className="vault-actions"><button className="vault-secondary" disabled={ep===1} onClick={()=>changeEpisode(ep-1)}>← Previous</button><button className="vault-primary" disabled={ep>=(animeData.episodes||1)} onClick={()=>changeEpisode(ep+1)}>Next Episode →</button></div></div><div className="source-row"><span><i/>{source?source.providerName+' · '+(source.quality||'Source available'):'No source available'}</span></div><div className="watch-episodes"><h2>Episodes</h2><div className="episode-grid">{Array.from({length:Math.min(animeData.episodes||0,12)},(_,i)=><button className={ep===i+1?'current':''} onClick={()=>changeEpisode(i+1)} key={i}>{i+1}</button>)}</div></div></section>;
}
...
export function AuthPage(){
 ...
 return <section className="auth-page"><div className="auth-backdrop"><div className="auth-orb auth-orb-one"/><div className="auth-orb auth-orb-two"/><div className="auth-grid"/></div>
  <div className="auth-shell"><Link className="auth-brand" to="/">{settings?.logo?.startsWith('http')||settings?.logo?.startsWith('/')?<img src={settings.logo} alt=""/>:<span>{settings?.logo||'✦'}</span>}<strong>{siteName}</strong></Link>
...
export function CustomPage(){const{settings}=useApp();return <section className="vault-page"><span className="vault-kicker">CUSTOM PAGE</span><h1>Created with {settings.siteName}.</h1><p>Customer-managed pages inherit the active {settings.siteName} visual system.</p></section>}

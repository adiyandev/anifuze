...
export function InstallerPage(){
 ...
 const next=async()=>{
  ...
  if(step===1){if(!licenseKey.trim()){setError('Enter your AniFuze product key to continue.');return}setBusy(true);try{const l=await api('/api/installer/license',{method:'POST',body:JSON.stringify({licenseKey})});setStatus(s=>s?{...s,license:l.license}:s);setStep(2)}catch(e){setError(e instanceof Error?e.message:'License verification failed')}finally{setBusy(false)}return}

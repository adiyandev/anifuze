...
import {Database,ShieldCheck,Server,CheckCircle2,AlertTriangle,ArrowRight,Loader2,KeyRound} from 'lucide-react';
...
export function InstallerPage(){
 ...
 const next=async()=>{
  ...
  if(step===1){if(!licenseKey.trim()){setError('Enter your AniFuze product key to continue.');return}setBusy(true);try{const s=await api('/api/installer/status');if(!s.requirements.ok)throw new Error('Fix the failed server requirements before continuing.');setStatus(s);setStep(2)}catch(e){setError(e instanceof Error?e.message:'Requirements check failed')}finally{setBusy(false)}return}
  if(step===3){if(!db.host||!db.name||!db.user){setError('Database host, name, and username are required.');return}setBusy(true);try{await api('/api/installer/database',{method:'POST',body:JSON.stringify(db)});setStep(3)}catch(e){setError(e instanceof Error?e.message:'Database connection failed')}finally{setBusy(false)}return}
  if(step===4){if(!admin.email.includes('@')||admin.password.length<12||admin.password!==admin.confirm){setError('Use a valid email, a password of at least 12 characters, and matching confirmation.');return}setBusy(true);try{await api('/api/installer/install',{method:'POST',body:JSON.stringify({database:db,admin,licenseKey})});setStep(4)}catch(e){setError(e instanceof Error?e.message:'Installation failed')}finally{setBusy(false)}}
 };
 const steps=['Welcome','Product key','Requirements','Database','Owner account','Complete'];

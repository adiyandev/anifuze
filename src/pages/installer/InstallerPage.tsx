...
export function InstallerPage(){
 ...
 const next=async()=>{
  setError('');
  if(step===0){await load();if(status?.locked)return;setStep(1);return}
 ...
 return <main className="installer-page"><section className="installer-card">
  ...
  {step===4&&<div className="installer-success"><CheckCircle2 size={54}/><h1>Installation complete</h1><p>AniFuze is installed and the installer is now locked.</p><a className="button" href="/admin/login">Open Admin Panel <ArrowRight size={16}/></a></div>}

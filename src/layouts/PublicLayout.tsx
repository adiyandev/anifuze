...
export function PublicLayout(){
 ...
 return <div className="vault-shell" style={{'--vault-primary':settings.primary,'--vault-accent':settings.accent} as CSSProperties}>
  <header className="vault-nav"><Link className="vault-brand" to="/"><span>{settings.logo||'✦'}</span><strong>{settings.siteName}</strong></Link>
   ...
   <div className="vault-nav-actions"><Link className="vault-search-trigger" to="/search"><Search size={16}/><span>Search anime...</span><kbd>⌘K</kbd></Link><Link className="vault-bell vault-notification-link" to="/notifications" aria-label="Notifications"><Bell size={16}/>{unreadNotifications>0&&<span className="vault-notification-badge">{unreadNotifications>99?"99+":unreadNotifications}</span>}</Link>{role!=='public_user'&&<Link className="vault-bell" to="/profile"><UserCircle size={16}/></Link>}{role==='public_user'?<Link className="vault-signin" to="/login"><UserCircle size={15}/> Sign in</Link>:<Link className="vault-avatar" to="/profile" aria-label="Open profile">A</Link>}<button className="vault-menu" onClick={()=>setOpen(!open)}>{open?<X size={19}/>:<Menu size={19}/>}</button></div>
  </header>

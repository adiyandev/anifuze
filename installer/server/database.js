...
export function toEnv(d,installationId,licenseKey,domain,providerMarketplaceUrl=''){
 return ['NODE_ENV=production','ANIFUZE_HTTPS=true','DB_CLIENT='+d.client,'DB_HOST='+d.host,'DB_PORT='+Number(d.port),'DB_NAME='+d.name,'DB_USER='+d.user,'DB_PASSWORD='+quote(d.password),'DB_SSL='+Boolean(d.ssl),'ANIFUZE_INSTALLATION_ID='+quote(installationId),'ANIFUZE_LICENSE_KEY='+quote(licenseKey),'ANIFUZE_DOMAIN='+quote(domain),'ANIFUZE_LICENSE_SERVICE_URL='+quote(process.env.ANIFUZE_LICENSE_SERVICE_URL||'https://animefusion.onrender.com'),'ANIFUZE_PROVIDER_MARKETPLACE_URL='+quote(providerMarketplaceUrl)].join('\\n')+'\\n';
}

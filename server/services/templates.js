...
export async function installTemplate(id){
 const remote=await getTemplate(id);
 if(!remote.id)throw new Error('Template was not found.');
 const now=new Date().toISOString();
 await query('INSERT INTO af_templates(id,name,version,status,config,installed_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$6) ON CONFLICT(id) DO UPDATE SET name=$2,version=$3,status=$4,config=$5,updated_at=$6',[remote.id,remote.name,remote.version,'installed',JSON.stringify(remote.config),now]);
 return getInstalledTemplate(remote.id);
}
export async function getInstalledTemplate(id){const r=await query('SELECT id,name,version,status,config,installed_at,updated_at FROM af_templates WHERE id=$1',[String(id)]);const x=r.rows[0];return x?{...x,config:parse(x.config)}:null;}
export async function listInstalledTemplates(){const r=await query('SELECT id,name,version,status,config,installed_at,updated_at FROM af_templates ORDER BY name ASC');return r.rows.map(x=>({...x,config:parse(x.config)}));}

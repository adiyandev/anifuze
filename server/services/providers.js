...
export async function testProvider(id){const p=await getProvider(id);if(!p)throw new Error('Provider not found.');const started=Date.now();let status='Healthy';let error=null;
 if(p.base_url){try{const u=new URL(p.base_url);if(!['http:','https:'].includes(u.protocol))throw new Error('URL must use HTTP or HTTPS.');const ac=new AbortController();const t=setTimeout(()=>ac.abort(),8000);const r=await fetch(u,{method:'GET',redirect:'manual',signal:ac.signal});clearTimeout(t);status=r.status<500?'Healthy':'Degraded';if(r.status>=400)error='HTTP '+r.status;}catch(e){status='Offline';error=e.name==='AbortError'?'Request timed out':e.message;}}

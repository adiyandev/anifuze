...
export async function listProviders(){const r=await query('SELECT id,name,type,base_url,config_json,credentials_encrypted,priority,enabled,status,latency_ms,request_count,error_count,last_checked_at,created_at,updated_at,owner_user_id,ownership,product_license_id,immutable_admin FROM af_providers ORDER BY priority ASC,name ASC');return r.rows.map(clean);}

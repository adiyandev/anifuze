...
export async function listAuditLogs({q='',action='',resourceType='',limit=50,offset=0}={}){
 ...
 if(q){values.push('%'+String(q).trim()+'%');where.push('(a.action ILIKE $'+values.length+' OR a.resource_type ILIKE $'+values.length+' OR a.resource_id ILIKE $'+values.length+' OR u.email ILIKE $'+values.length+')');}

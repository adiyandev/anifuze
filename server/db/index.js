...
export async function query(text,params=[]){
 const p=getPool(); if(config.db.client==='postgres')return p.query(text,params);
 const conn=await p.getConnection(); try{const rows=await conn.query(text.replace(/\$(\d+)/g,'?'),params);return {rows:Array.isArray(rows)?rows:[]};}finally{conn.release?.();}
}
export async function healthCheck(){await query('SELECT 1');return {ok:true,client:config.db.client};}
export async function listAniFuzeTables(){
 const rows=await query(config.db.client==='postgres'?"SELECT table_name AS name FROM information_schema.tables WHERE table_schema='public' AND table_name LIKE 'af_%'":"SELECT table_name AS name FROM information_schema.tables WHERE table_schema=DATABASE() AND table_name LIKE 'af_%'");
 return rows.rows.map(r=>r.name);
}
export async function dropAniFuzeTables(names){
 for(const name of [...new Set(names)].filter(n=>/^af_[A-Za-z0-9_$-]+$/.test(n)).reverse()){
  await query(config.db.client==='postgres'?'DROP TABLE IF EXISTS "'+name+'" CASCADE':'DROP TABLE IF EXISTS '+String.fromCharCode(96)+name+String.fromCharCode(96));
 }
}

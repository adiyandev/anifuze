...
import {requireAdmin} from './auth.js'; import {requirePermission} from '../auth/permissions.js'; import {query} from '../db/index.js';
...
router.patch('/admin/anilist/anime/:id',requirePermission('anime_manage'),async(req,res)=>{
  ...
  if(!existing.rows[0])return res.status(404).json({ok:false,error:'Anime not found.'});
  const body=req.body||{};
